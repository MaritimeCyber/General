  ...
  (function () {
    ...
    var CACHE_KEY = "smj_cache_v2"; /* bumped: v1 could have cached a truncated (45-post) result from the pagination bug */
    ...
    function smjShareCard(btn, e) {
      if (window.scsShareCard) { window.scsShareCard(btn, e); return; }
      e.stopPropagation();
      e.preventDefault();
      var prev = document.getElementById("smj-sp");
      if (prev) {
        prev.remove();
        if (window._smjSPBtn === btn) { window._smjSPBtn = null; return; }
      }
      window._smjSPBtn = btn;
      ...
      function closePanel() {
        var p = document.getElementById("smj-sp");
        if (p) p.remove();
        window._smjSPBtn = null;
      }
    ...
    window.smjShareCard = smjShareCard;

    function cardHTML(p) {
      var thumbInner = p.thumb
        ? '<div aria-label="' + escAttr(p.title) + '" class="smj-card-thumb" role="img" style="background-image:url(\'' + escAttr(p.thumb) + "');\"></div>"
        : '<div class="smj-card-thumb smj-card-noimg"></div>';
      var readBadge = readSet[normUrl(p.link)] ? '<span class="smj-card-read-badge">✓ Read</span>' : "";
      var verifyHTML = verifyBadgeHTML(p.labels, "smj-card-verify-badge");
      var thumbHTML = '<div class="smj-card-thumb-wrap">' + thumbInner + readBadge + verifyHTML + "</div>";

      var descHTML = p.summary ? '<div class="smj-card-desc">' + escHTML(p.summary) + "…</div>" : "";
      ...
      if (p.labels && p.labels.length) {
        ...
        var moreHTML = p.labels.length > MAX_TAGS ? '<span class="smj-card-tag-more" title="' + escAttr(p.labels.slice(MAX_TAGS).join(", ")) + '">…</span>' : "";
        tagsHTML =
          '<div class="smj-card-tags">' +
          visibleLabels
            .map(function (l) {
              return (
                '<span class="smj-card-tag" data-label="' + escAttr(l) + '" ' +
      ...
      var authorHTML = "";
      if (p.authorImg) authorHTML += '<img alt="' + escAttr(p.author) + '" class="smj-card-avatar" loading="lazy" src="' + escAttr(p.authorImg) + '" />';
      if (p.author) authorHTML += '<span class="smj-card-author-name">' + escHTML(p.author) + "</span>";

      var commentsHTML =
        '<span class="smj-card-comments" onclick="event.preventDefault();event.stopPropagation();location.href=this.closest(\'a\').href+\'#comment-form\';">💬 ' +
        (p.comments || 0) +
        "</span>";

      var shareHTML =
        '<button class="smj-card-share spj-card-share" data-url="' + escAttr(p.link) + '" data-title="' + escAttr(p.title) + '" ' +
        'onclick="smjShareCard(this,event)" type="button">' +
        SHARE_SVG +
        "</button>";

      return (
        '<a class="smj-card" href="' + escAttr(p.link) + '" rel="noopener" target="_blank">' +
        thumbHTML +
        '<div class="smj-card-body">' +
        '<h4 class="smj-card-title">' + escHTML(p.title) + "</h4>" +
        descHTML +
        tagsHTML +
        '<div class="smj-card-footer">' +
        '<div class="smj-card-author-row">' + authorHTML + '<span class="smj-card-date">' + fmtCardDate(p.published) + "</span></div>" +
        '<div class="smj-card-footer-right">' + commentsHTML + shareHTML + "</div>" +
        "</div></div></a>"
      );
    }

    /*── Carousel row markup (one per week — 3.5 cards per line, matching
    ...
    function carouselHTML(posts) {
      ...
      return (
        '<div class="smj-carousel-wrap">' +
        '<button class="smj-car-btn smj-car-prev" disabled type="button" aria-label="Previous"><span class="smj-car-arrow"></span></button>' +
        '<div class="smj-car-outer"><div class="smj-car-track">' + posts.map(cardHTML).join("") + "</div></div>" +
        '<button class="smj-car-btn smj-car-next" disabled type="button" aria-label="Next"><span class="smj-car-arrow"></span></button>' +
        "</div>" +
        '<div class="smj-car-indicator">' + indicator + "</div>"
      );
    }

    function renderArchive(items) {
      refreshReadSet();
      var archiveEl = document.getElementById("smj-archive");
      var emptyEl = document.getElementById("smj-empty");
      var yearJumpEl = document.getElementById("smj-year-jump");
      ...
      yearJumpEl.innerHTML = years
        .map(function (y, idx) {
          ...
          return '<a class="smj-year-pill" data-year="' + y + '" href="#smj-year-' + y + '">' + y + ' <span class="smj-cnt">' + count + "</span></a>";
        })
        .join("");
      ...
      archiveEl.innerHTML = years
        .map(function (y, yIdx) {
          ...
          var monthsHTML = months
            .map(function (m) {
              var weeks = Object.keys(tree[y][m]).map(Number).sort(function (a, b) { return a - b; });
              var monthCount = 0;
              var weeksHTML = weeks
                .map(function (w) {
                  ...
                  return (
                    '<div class="smj-week-label"><span>' + weekRangeLabel(m, w, y) + '</span><span class="smj-week-line"></span></div>' +
                    carouselHTML(posts)
                  );
                })
                .join("");
              ...
              return (
                '<div class="smj-month-header"><h3 class="smj-month-name">' + MONTHS[m] + '</h3><span class="smj-month-count">' + monthCount + " posts</span></div>" +
                weeksHTML
              );
            })
            .join("");
          return (
            '<details class="smj-year-block" id="smj-year-' + y + '"' + (yIdx === 0 ? " open" : "") + '>' +
            '<summary class="smj-year-summary"><span class="smj-year-chevron">▶</span><h2 class="smj-year-title">' + y + '</h2><span class="smj-year-badge">' + yearCount + " posts</span></summary>" +
            '<div class="smj-year-body">' + monthsHTML + "</div>" +
            "</details>"
          );
        })
        .join("");
    ...
    function initAllCarousels() {
      carousels = [];
      document.querySelectorAll("#smj-archive .smj-carousel-wrap").forEach(function (wrap) {
        var track = wrap.querySelector(".smj-car-track");
        var outer = wrap.querySelector(".smj-car-outer");
        var prevBtn = wrap.querySelector(".smj-car-prev");
        var nextBtn = wrap.querySelector(".smj-car-next");
        var indEl = wrap.nextElementSibling && wrap.nextElementSibling.classList.contains("smj-car-indicator") ? wrap.nextElementSibling : null;
      ...
      /* Collapsed <details> years render their content at 0 width (the
         ...
         width. */
      document.querySelectorAll("#smj-archive .smj-year-block").forEach(function (details) {
    ...
    function loadTopRanked(items) {
      var loadingEl = document.getElementById("smj-ranked-loading");
      var listEl = document.getElementById("smj-ranked-list");
      if (!items.length) {
        ...
        listEl.innerHTML = top
          .map(function (r, i) {
            var thumbHTML = r.post.thumb
              ? '<span class="smj-rank-thumb" style="background-image:url(\'' + escAttr(r.post.thumb) + "');\"></span>"
              : '<span class="smj-rank-thumb smj-rank-noimg"></span>';
            var avatarHTML = r.post.authorImg
              ? '<span class="smj-rank-avatar" style="background-image:url(\'' + escAttr(r.post.authorImg) + "');\"></span>"
              : "";
            var authorHTML = r.post.author ? avatarHTML + '<span class="smj-rank-author">' + escHTML(r.post.author) + "</span>" : "";
            var metaHTML =
              authorHTML +
              '<span class="smj-rank-date">' + fmtCardDate(r.post.published) + "</span>" +
              '<span class="smj-rank-comments">💬 ' + r.post.comments + "</span>";
            var verifyHTML = verifyBadgeHTML(r.post.labels, "smj-rank-verify-badge");
            return (
              '<a class="smj-rank-row" href="' + escAttr(r.post.link) + '" rel="noopener" target="_blank">' +
              '<span class="smj-rank-num">' + (i + 1) + "</span>" +
              thumbHTML +
              '<span class="smj-rank-body"><div class="smj-rank-title-row"><h4 class="smj-rank-title">' + escHTML(r.post.title) + "</h4>" + verifyHTML + '</div><div class="smj-rank-meta">' + metaHTML + "</div></span>" +
              "</a>"
            );
          })
          .join("");
    ...
    function onData(items) {
      ...
      allItems = items;
      document.getElementById("smj-loading").style.display = "none";
      renderArchive(filterByPeriod(allItems));
    ...
    document.getElementById("smj-search-input").addEventListener("input", function () {
    ...
    /*── Year pills (under the search box): clicking one opens only that
         ...
         #smj-year-jump's innerHTML is regenerated on every renderArchive
         call — a listener bound directly to a pill would be lost on the
         next re-render. ──*/
    document.getElementById("smj-year-jump").addEventListener("click", function (e) {
      var pill = e.target.closest(".smj-year-pill");
      if (!pill) return;
      e.preventDefault();
      var year = pill.getAttribute("data-year");
      document.querySelectorAll("#smj-archive .smj-year-block").forEach(function (details) {
        details.open = details.id === "smj-year-" + year;
      });
      var target = document.getElementById("smj-year-" + year);
    ...
    document.getElementById("smj-period-bar").addEventListener("click", function(e) {
      var btn = e.target.closest(".spj-period-btn");
      if (!btn) return;
      document.querySelectorAll("#smj-period-bar .spj-period-btn").forEach(function(b) { b.classList.remove("active"); });
